...
& 'C:\Users\ZGAMESVN\Downloads\Soccer-Mobile-Pro\Recover-SoccerUnity.ps1' -PreflightOnly

# Synthetic tests plus installed AssetRipper API and Unity assembly checks.
# Does not load or extract Soccer-Mobile-Pro.apk.
& 'C:\Users\ZGAMESVN\Downloads\Soccer-Mobile-Pro\Test-RecoveryPipeline.ps1'

# Perform recovery and Unity import/compilation/asset validation.
& 'C:\Users\ZGAMESVN\Downloads\Soccer-Mobile-Pro\Recover-SoccerUnity.ps1'
...
The project and recovery directories must be absent or empty. Existing and failed outputs are retained. Inspect and archive an earlier run before using the same fixed paths again; there is deliberately no destructive cleanup switch. Each processing stage defaults to a two-hour timeout, adjustable through `-StageTimeoutSeconds`.

After a successful run, select **Add project from disk** in Unity Hub and choose `C:\Users\ZGAMESVN\Downloads\Soccer-Unity-Project`, using editor **2020.3.49f1**.

## Exact inputs and processing

| Input | Fixed location |
| --- | --- |
| APK | `C:\Users\ZGAMESVN\Downloads\Soccer-Mobile-Pro\Soccer-Mobile-Pro.apk` |
| Java evidence | `C:\Users\ZGAMESVN\Downloads\Soccer-Mobile-Pro\app\src\main\java` |
...
4. Run a private AssetRipper copy as a hidden loopback service. Use `/Settings/Update`, `/LoadFile`, and `/Export/UnityProject`; verify settings persisted before loading. Request `Level1` method stubs, `Decompiled` C# output, C# 9, shader decompilation, and StreamingAssets extraction.
...
```text
C:\Users\ZGAMESVN\Downloads\Soccer-Unity-Project\
  Assets\
...
**The APK recovery pipeline has not been executed.** Successful extraction, package resolution, project import, and compilation of this particular recovered game are not yet established. The synthetic tests and API tests do not substitute for that end-to-end run. `Test-RecoveryPipeline.ps1` leaves its synthetic inputs and logs under uniquely named directories in the Windows temporary folder for inspection.
