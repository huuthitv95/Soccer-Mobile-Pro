...
try {
    ...
    Invoke-RipperRequest $base '/Settings/Update' @{
        ...
        ScriptLanguageVersion='CSharp9_0';StreamingAssetsMode='Extract';ShaderExportMode='Decompile';
        ScriptTypesFullyQualified='on';RemoveNullableAttributes='on';ExportUnreadableAssets='on';SaveSettingsToDisk='on'
    } 60
    $saved=Get-Content -LiteralPath (Join-Path $copy 'AssetRipper.Settings.json') -Raw | ConvertFrom-Json
